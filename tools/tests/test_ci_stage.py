...
class RestoredSourceUpdateRegressionTest(unittest.TestCase):
    ...
    def test_interrupted_chromix_patch_layer_resumes_without_discarding_source(self):
        ...
        reverse_check = prepare.index('& $PatchExe -p1 --batch --reverse --dry-run -i $patch')
        reverse_recovery = prepare.index('& $PatchExe -p1 --batch --reverse --force -i $patch')
        ...
        self.assertLess(reverse_recovery, forward_check)
        self.assertLess(forward_check, normal_forward)
