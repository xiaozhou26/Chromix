// tilion-fortress (Node) — CloakBrowser-compatible API surface.
//
// Mirrors the `cloakbrowser` JS wrapper (github.com/CloakHQ/CloakBrowser) so
// existing CloakBrowser scripts run on Fortress by changing only the import:
//
//   - import { launch } from "cloakbrowser";
//   + import { launch } from "tilion-fortress";
//
// Same function names, same option names (camelCase, contextOptions/
// launchOptions nesting), same return types (Playwright Browser /
// BrowserContext via playwright-core — imported lazily so the SDK itself stays
...
// puppeteer subpath yet (use the playwright surface).
...
function chromeBinaryPath(plat, tag) {
  ...
  return join(CACHE, tag, plat, "tilion-fortress", name);
}
...
export async function ensureBinary({ browserVersion, releaseChannel } = {}) {
  ...
  if (!plat) throw new Error("No native Fortress binary for this platform; use the Docker image or Linux/Windows x64.");
...
export async function checkForUpdate() {
  ...
  let latest = current;
  try {
    const r = await fetch("https://api.github.com/repos/tiliondev/fortress/releases/latest",
...
export async function maybeResolveGeoip(geoip, proxy, timezone, locale, args) {
  ...
  if (!geo) {
    process.stderr.write("[tilion-fortress] geoip lookup failed; timezone/locale unchanged\n");
...
// ---------------------------------------------------------------------------
// Widevine CDM location (env or installed Chrome; Linux deb fetch lives in
// the Python tool — Node has no stdlib xz)
// ---------------------------------------------------------------------------
...
export function findWidevineCdm() {
  const plat = resolvePlatform();
  if (!plat) return null;
  for (const env of ["CLOAKBROWSER_WIDEVINE_CDM", "TILION_WIDEVINE_CDM", "FORTRESS_WIDEVINE_CDM"]) {
    const p = process.env[env];
    if (p) {
      if (validCdmDir(p, plat)) return p;
      console.warn(`[tilion-fortress] ${env}=${p} is not a valid CDM dir; ignoring`);
    }
  }
  const roots = {
...
export function buildContextOptions(options = {}) {
  ...
  if (locale !== undefined || timezoneId !== undefined)
    console.warn("[tilion-fortress] contextOptions.locale/timezoneId ignored — use top-level locale/timezone (binary flag)");
...
export async function buildLaunchOptions(options = {}) {
  ...
  // Widevine: explicit dir wins, else auto-detect an installed Chrome's CDM.
  const widevineDir = options.widevineCdm ||
    (options.widevine || process.env.CLOAKBROWSER_WIDEVINE === "1" ? findWidevineCdm() : null);
  if (widevineDir) args = [...(args || []), `--uxr-widevine-cdm=${widevineDir}`];
