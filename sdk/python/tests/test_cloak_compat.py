"""CloakBrowser-compatibility layer tests (no browser launch needed)."""
import sys
import time
...
from tilion_fortress import cloak  # noqa: E402
from tilion_fortress.humanize import Humanizer, patch_page, resolve_human_config  # noqa: E402
...
def test_cloak_api_import_surface():
    import tilion_fortress as tf
    for name in cloak.__all__:
        assert getattr(tf, name, None) is not None, name


def test_proxy_string_with_credentials():
    pw, extra = cloak._resolve_proxy_config("http://u:p@proxy:8080")
    assert pw == {"proxy": {"server": "http://proxy:8080", "username": "u", "password": "p"}}
    assert extra == []


def test_proxy_string_plain():
    pw, _ = cloak._resolve_proxy_config("socks5://proxy:1080")
    assert pw == {"proxy": {"server": "socks5://proxy:1080"}}


def test_proxy_dict_passthrough():
    pw, _ = cloak._resolve_proxy_config(
...
def test_extract_proxy_url():
    assert cloak._extract_proxy_url(None) is None
    assert cloak._extract_proxy_url("http://p:1") == "http://p:1"
    assert cloak._extract_proxy_url(
        {"server": "http://p:1", "username": "u", "password": "p"}) == "http://u:p@p:1"


def test_build_args_priority_and_dedupe():
    args = cloak.build_args(
        True,
    ...
    # headless + non-Windows keeps the blocklist flag off here only when
    # explicitly disabled; build_args adds it on Windows or headed. Just check
...
def test_build_args_maximize_suppressed_by_geometry():
    args = cloak.build_args(True, ["--window-size=800,600"], start_maximized=True)
    assert "--start-maximized" not in args
    args = cloak.build_args(True, None, start_maximized=True)
    assert "--start-maximized" in args


def test_get_default_stealth_args_seed_shape():
    sa = cloak.get_default_stealth_args()
...
def test_geoip_disabled_passthrough():
    assert cloak.maybe_resolve_geoip(False, None, "UTC", "en") == ("UTC", "en", None)
...
def test_geoip_flag_promotion(monkeypatch):
    ...
    monkeypatch.setattr(cloak, "_geoip_http", fake_http)
    # Raw flags count as explicit: geoip must not clobber them.
    tz, loc, ip = cloak.maybe_resolve_geoip(
...
def test_webrtc_auto_resolution(monkeypatch):
    monkeypatch.setattr(cloak, "_geoip_http", lambda p: (None, None, "198.51.100.7"))
    out = cloak._resolve_webrtc_args(["--fingerprint-webrtc-ip=auto"], "http://p:1")
    assert out == ["--fingerprint-webrtc-ip=198.51.100.7"]
    out = cloak._resolve_webrtc_args(["--fingerprint-webrtc-ip=auto"], None)
    assert out == []


def test_webrtc_exit_ip_append():
    out = cloak._append_webrtc_exit_ip(["--headless=new"], "198.51.100.7")
    assert out[-1] == "--fingerprint-webrtc-ip=198.51.100.7"
    out = cloak._append_webrtc_exit_ip(["--fingerprint-webrtc-ip=x"], "198.51.100.7")
    assert out == ["--fingerprint-webrtc-ip=x"]
    assert cloak._append_webrtc_exit_ip(None, None) is None
